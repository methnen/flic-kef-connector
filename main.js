// flic-kef-connector — Flic Hub Studio module
// Maps a Flic Twist (registered as a Virtual Speaker) for volume changes and click gestures to a KEF gen-2 speaker
//
// Configure the two constants below for your environment, then deploy

var KEF_IP = "192.168.1.129";
var SPEAKER_VIRTUAL_DEVICE_ID = "KEF"; // Match to any Speaker virtual device

// Message strings sent from the Flic app's "Send message to Hub Studio" action for each Twist click gesture
// Edit these to differentiate between Flic Twist controllers and multiple speakers
...
function scheduleVolume(target) {
    ...
    volumeTimer = setTimeout(function () {
        ...
        kef.setVolume(value, function (error) {
            if (error) {
                console.log("[kef] setVolume(" + value + ") failed: " + error);
            };
        });
    }, VOLUME_DEBOUNCE_MS);
}
...
flicapp.on("virtualDeviceUpdate", function (meta, values) {
    if (rotationEventsLogged < DEBUG_LOG_FIRST_EVENTS) {
        console.log("[twist] virtualDeviceUpdate meta=" + JSON.stringify(meta) + " values=" + JSON.stringify(values));
        rotationEventsLogged++;
    }

    if (!meta || meta.dimmableType !== "Speaker") {
        return;
    }

    if (SPEAKER_VIRTUAL_DEVICE_ID && meta.virtualDeviceId !== SPEAKER_VIRTUAL_DEVICE_ID) {
        return;
    }

    if (!values || typeof values.volume !== "number") {
        return;
    }
...
flicapp.on("actionMessage", function (message) {
    ...
    if (message === MSG_PLAY_PAUSE) {
        ...
        kef.sendControl("pause", function (error) {
            if (error) {
                console.log("[kef] sendControl(pause) failed: " + error);
            }
        });
    } else if (message === MSG_NEXT) {
        kef.sendControl("next", function (error) {
            if (error) {
                console.log("[kef] sendControl(next) failed: " + error);
            }
        });
    } else if (message === MSG_PREVIOUS) {
        kef.sendControl("previous", function (error) {
            if (error) {
                console.log("[kef] sendControl(previous) failed: " + error);
            }
        });
    }
});
...
function probeSpeaker(attempt) {
    kef.getPlayerData(function (error, data) {
        ...
        kef.getVolume(function (volumeError, volume) {
            ...
            console.log("[kef] connected to " + KEF_IP + " — volume=" + volume + " state=" + (data && data.state));
        });
    });
}
